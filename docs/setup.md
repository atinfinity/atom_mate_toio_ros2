...
| 項目 | 内容 |
|---|---|
| Wi-Fi アクセスポイント / SSID | ATOM Matrix を接続する Wi-Fi(2.4GHz 帯のみ)。一覧(電波の強い順)から選ぶと SSID 欄に入る。一覧に無ければ「再スキャン」するか SSID 欄に直接入力 |
| Wi-Fi パスワード | パスワードなしの AP なら空欄。SSID が前回と同じなら空欄のままで前回のパスワードを維持 |
...
**設定をやり直す**には、ATOM Matrix のボタンを押しながら USB を挿し直す(または押しながらリセット)と、再び設定モードに入ります。パスワード以外は前回の値が初期表示され、SSID を変えない限りパスワードは空欄のままで維持されます。
